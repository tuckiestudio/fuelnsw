...
// Generate prices with some variety
function generatePrices() {
	...
	for (const station of MOCK_STATIONS) {
		for (const fuel of fuelTypes) {
			...
			// Add some random variation (±20 cents)
			const variation = Math.round((Math.random() - 0.5) * 40);
			const price = base + variation;

			// Some stations don't have all fuel types
			if (station.code === 'S004' && fuel === 'P98') continue; // United Manly doesn't sell P98
			if (station.code === 'S007' && fuel === 'P95') continue; // Metro Petroleum doesn't sell P95
			if (station.code === 'S012' && fuel === 'E10') continue; // Shell Wagga doesn't sell E10
			if (station.code === 'S003' && fuel === 'Diesel') continue; // Shell Bondi "running dry" on Diesel (mock dry station)
...
// Generate historical data (last 90 days, every 3 days for mock data)
function generateHistoricalData() {
	...
	const transaction = db.transaction(() => {
		for (const station of MOCK_STATIONS) {
			for (const fuel of fuelTypes) {
				// Skip same exclusions as live prices
				...
				lastSeen.setDate(lastSeen.getDate() - Math.floor(Math.random() * 5)); // 0-4 days ago

				let dateCount = 0;
				for (let d = new Date(firstDate); d <= today; d.setDate(d.getDate() + 3)) {
					dateCount++;
					// Price trend: slowly increasing with noise
				...
				// All stations get normal inventory first
				insertInv.run(station.code, fuel, firstDate.toISOString().slice(0, 10), lastSeen.toISOString().slice(0, 10));

				// Override: Mark "S003 Shell Bondi" as potentially dry for Diesel
