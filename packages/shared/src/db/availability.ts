...
export function detectAndRecordChanges(
	...
	const upsertPendingStmt = db.prepare(
		`INSERT INTO pending_drops (station_code, fuel_type, miss_count, first_missing_at, last_missing_at, previous_price)
		 VALUES (?, ?, 1, datetime('now'), datetime('now'), ?)
		 ON CONFLICT(station_code, fuel_type) DO UPDATE SET
			miss_count = pending_drops.miss_count + 1,
			last_missing_at = datetime('now'),
			previous_price = excluded.previous_price`
	);

	const clearPendingStmt = db.prepare(
		`DELETE FROM pending_drops WHERE station_code = ? AND fuel_type = ?`
	);

	const getPendingStmt = db.prepare(
		`SELECT miss_count, previous_price FROM pending_drops WHERE station_code = ? AND fuel_type = ?`
	);
	...
	const tx = db.transaction(() => {
		for (const [key, val] of before) {
			if (!after.has(key)) {
				...
				upsertPendingStmt.run(station_code, fuel_type, val.price);

				const pending = getPendingStmt.get(station_code, fuel_type) as { miss_count: number; previous_price: number } | undefined;
				if (pending && pending.miss_count >= CONSECUTIVE_MISS_THRESHOLD) {
					insertEventStmt.run(station_code, fuel_type, 'dropped', pending.previous_price, refreshId);
					dropped++;
					clearPendingStmt.run(station_code, fuel_type);
				}
			}
		}
		...
		for (const key of after.keys()) {
			const [station_code, fuel_type] = key.split('|');
			if (!before.has(key)) {
				insertEventStmt.run(station_code, fuel_type, 'added', null, refreshId);
				added++;
			}
			clearPendingStmt.run(station_code, fuel_type);
		}
...
export function cleanupStalePendingDrops(): number {
	const db = getDb();

	const result = db.prepare(`
		DELETE FROM pending_drops
		WHERE last_missing_at < datetime('now', '-1 hour')
	`).run();
