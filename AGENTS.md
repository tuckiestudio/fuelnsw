...
│       ├── utils/            # fuel-types.ts, parse-address.ts, discounts.ts
...
│       ├── components/map/   # SearchBar, FuelTypeSelector, StationPanel, QuickFuel*, DiscountModal, etc.
│       └── lib/              # preferences, navigation, ads, subscription, discount-state.svelte.ts
...
|---|---|---|
| `/api/fuel/stations` | GET | All stations with live prices as GeoJSON (optional `open_only`) |
| `/api/fuel/stations/viewport?south,west,north,east,fuel,open_only` | GET | Stations within map bounds |
| `/api/fuel/stations/nearest?lat,lng,fuel,limit,radius,open_only` | GET | Nearest cheapest stations (default 20km, sorted by price) |
...
- **Maps** — `leaflet` + `leaflet.markercluster`. Markers keyed by station code in `markerMap` and `stationMap` for hover sync with `StationListPanel`.
...
- `z-[1003]`: Panels (StationPanel, StationListPanel, QuickFuelSheet) — mutually exclusive
- `z-[1004]`: System overlays (loading, errors)
- `z-[2000]`: Onboarding modal
- `z-[2001]`: Discount modal

### Station list and map hover sync
Hovering a station in `StationListPanel` opens a standalone `L.tooltip` at the station's coordinates on the map and highlights the list row. Hovering a map marker sets `hoveredStationCode` to highlight the matching list row. Markers are stored in `markerMap` (code → marker) and station data in `stationMap` (code → StationGeoJSON), both populated in `renderMarkers()`.

### Advertising and subscriptions
- **Web**: AdSense display ads in StationPanel and QuickFuelSheet. `adsRemoved` is always `false`. No removal option.
- **Native (iOS/Android)**: AdMob banner (adaptive, bottom center) + interstitials (every 3rd trigger). RevenueCat in-app subscription ($1/mo) removes all ads. No backend subscriber tracking — RevenueCat is source of truth.
- **AdSlot component** (`src/components/AdSlot.svelte`): Renders AdSense `<ins>` on web, hidden on native. In dev mode shows a dashed placeholder instead (AdSense can't serve on localhost).
- **ads.txt**: Served from `apps/web/static/ads.txt` for AdSense verification.
- **Ad IDs**: All in `src/lib/ads.ts` — interstitial, banner (iOS/Android), and web AdSense slot.
- **Analytics**: GA4 (`G-TN0H67V2B2`) via gtag.js in `app.html`.

### User preferences
`src/lib/preferences.ts` stores in localStorage: selected fuel type (default `E10`), last map position, onboarding state, selected discount IDs, open-only filter, preferred navigation app.

### Fuel discounts
Pure frontend feature — no DB or API changes. All discount data and calculation logic in `packages/shared/src/utils/discounts.ts` (30 offers, 10 brand configs, stacking engine). Uses `station.brand` (NOT `brand_group`) for discount lookup.

- **Reactive state**: `apps/web/src/lib/discount-state.svelte.ts` — Svelte 5 `.svelte.ts` module with `$state`/`$derived`. Exports getter functions (`getSelectedDiscounts()`, `getShowDiscountModal()`, `getDiscountCount()`) because Svelte 5 does not allow exporting reassigned `$state` variables from modules. Also exports `onDiscountModalClose()` for registering callbacks when the modal closes (used by `+page.svelte` to refresh markers).
- **DiscountModal** (`components/map/DiscountModal.svelte`): Full-screen selector at `z-[2001]` with grouped toggles. Footer shows oldest `lastVerified` date and source attribution link to OzBargain Fuel Discounts Wiki.
- **Integration points**: Map markers (`+page.svelte`) show discounted price + badge, StationPanel shows per-fuel breakdown + applied discounts section, QuickFuelSheet re-sorts by discounted price, StationListPanel shows discount badge.
- **7-Eleven Price Lock**: Skipped for v1.
- **Maintenance**: See `docs/fuel-discounts-plan.md` line 446 for review checklist, `docs/fuel-discounts-review.md` for audit results. To update a discount, edit `DISCOUNT_OFFERS` in `discounts.ts`. To change brand acceptance, edit `BRAND_CONFIGS`. Code deploy required — no runtime data.
...
10. **`adsRemoved` on web** — Hardcoded `false` in `+layout.svelte`. Only read from localStorage on native.
	...
	12. **Viewport layout** — `+layout.svelte` uses `h-dvh` (100dvh) with `flex flex-col` so nav + main + footer fit within the viewport with no page-level scroll. Map page uses `h-full` to fill the main area. Other pages (e.g. summary) scroll within the `overflow-auto` main element.

	13. **`better-sqlite3` must be in web `package.json`** — `adapter-node` only externalizes packages in web's own `dependencies`. If transitive only, Rollup bundles it and crashes with `__filename is not defined`.

	14. **All Capacitor packages in web `package.json`** — Needed at build time even though guarded by `Capacitor.isNativePlatform()` at runtime.

	15. **npm workspace hoisting** — Dependencies installed at repo root `/app/node_modules`, not per-app. Dockerfile copies from root.

	16. **Android AdMob app ID** — Real publisher ID (`ca-app-pub-8792853309353392`) in `AndroidManifest.xml`. iOS has the same in `Info.plist`.

	17. **Svelte 5 `$state` exports from `.svelte.ts` modules** — Cannot export `$state` variables that are reassigned (e.g. `selectedDiscounts = [...]`). Svelte compiler throws `state_invalid_export`. Use getter functions instead (`getSelectedDiscounts()`, `getShowDiscountModal()`). The getters are still reactive in consuming components because Svelte 5 tracks `$state` access through function calls.
