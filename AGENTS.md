...
`QuickFuelButton` → locate user → `/api/fuel/stations/nearest` → `QuickFuelSheet` bottom sheet. Opens Apple Maps on iOS, Google Maps on Android/web.
...
9. **Pre-existing type errors** — `svelte-check` reports `@fuelnsw/shared` module resolution errors. Vite resolves at runtime. Do not fix.
