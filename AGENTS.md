...
### User preferences
`src/lib/preferences.ts` stores in localStorage: selected fuel type (default `E10`), last map position, onboarding state, selected discount IDs.

### Fuel discounts
Pure frontend feature — no DB or API changes. All discount data and calculation logic in `packages/shared/src/utils/discounts.ts` (28 offers, 9 brand configs, stacking engine). Uses `station.brand` (NOT `brand_group`) for discount lookup.

- **Reactive state**: `apps/web/src/lib/discount-state.svelte.ts` — Svelte 5 `.svelte.ts` module with `$state`/`$derived`. Exports getter functions (`getSelectedDiscounts()`, `getShowDiscountModal()`, `getDiscountCount()`) because Svelte 5 does not allow exporting reassigned `$state` variables from modules.
- **DiscountModal** (`components/map/DiscountModal.svelte`): Full-screen selector at `z-[2001]` with grouped toggles. Footer shows oldest `lastVerified` date.
...
- **Maintenance**: See `docs/fuel-discounts-plan.md` line 446 for review checklist. To update a discount, edit `DISCOUNT_OFFERS` in `discounts.ts`. To change brand acceptance, edit `BRAND_DISCOUNT_CONFIGS`. Code deploy required — no runtime data.
...
10. **`adsRemoved` on web** — Hardcoded `false` in `+layout.svelte`. Only read from localStorage on native.
	...
	12. **`better-sqlite3` must be in web `package.json`** — `adapter-node` only externalizes packages in web's own `dependencies`. If transitive only, Rollup bundles it and crashes with `__filename is not defined`.

	13. **All Capacitor packages in web `package.json`** — Needed at build time even though guarded by `Capacitor.isNativePlatform()` at runtime.

	14. **npm workspace hoisting** — Dependencies installed at repo root `/app/node_modules`, not per-app. Dockerfile copies from root.

	15. **Android AdMob app ID** — Real publisher ID (`ca-app-pub-8792853309353392`) in `AndroidManifest.xml`. iOS has the same in `Info.plist`.

	16. **Svelte 5 `$state` exports from `.svelte.ts` modules** — Cannot export `$state` variables that are reassigned (e.g. `selectedDiscounts = [...]`). Svelte compiler throws `state_invalid_export`. Use getter functions instead (`getSelectedDiscounts()`, `getShowDiscountModal()`). The getters are still reactive in consuming components because Svelte 5 tracks `$state` access through function calls.
