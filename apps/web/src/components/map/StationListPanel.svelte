<script lang="ts">
	...
	function getDiscountedPrice(station: StationGeoJSON): number {
		...
		const discount = calculateTotalDiscount(station.properties.brand || '', fuelType, getSelectedDiscounts(), station.properties.code, raw === Infinity ? undefined : raw, getGiftCardEnabledState() ? getGiftCardPercentState() : undefined, getGiftCardBrandsState() ?? undefined);
		...
		{:else}
			<div class="divide-y divide-gray-100">
				{#each sortedStations as station, i}
					...
					{@const discount = calculateTotalDiscount(station.properties.brand || '', fuelType, getSelectedDiscounts(), station.properties.code, price === Infinity ? undefined : price, getGiftCardEnabledState() ? getGiftCardPercentState() : undefined, getGiftCardBrandsState() ?? undefined)}
